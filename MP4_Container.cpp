...
void CMP4_Container::load(FILE *Stream, u32 offset, u32 size)
{
	...
	{
		_fseeki64(Stream, _offset, SEEK_SET);
		...
		if (atomSize == 0) // This is the last atom, it reaches to the end of the file
		{
			if ((u32)CTools::FileSize <= _offset)
				return;
