...
int COggVorbis::GetSamples(FILE *Source)
{
	...
	for (Index = 1; Index < 50; Index++)
	{
		...
		_fseeki64(Source, DataIndex, SEEK_SET);
		...
		for (Iterator = (251 - 10); Iterator > 0; Iterator--)
		{
			...
			{
				_fseeki64(Source, DataIndex + Iterator, SEEK_SET);
...
void COggVorbis::SetLacingValues()
{
	...
	Value = Value +	(long)Data.GetLength() - (long)(TagEndPos - SPagePos - SPage.Segments - 27);
	if (Value < 0)
		Value = 0;
...
void COggVorbis::SetCRC(FILE *Destination)
{
	...
	_fseeki64(Destination, SPagePos + SPage.Segments + 27, SEEK_SET);
	for (Index = 0; Index < SPage.Segments; Index++)
	{
	...
	_fseeki64(Destination, SPagePos + 22, SEEK_SET);
...
bool COggVorbis::RebuildFile(LPCWSTR FileName)
{
	...
	tmp.FileRead((size_t)SPagePos, Source);
