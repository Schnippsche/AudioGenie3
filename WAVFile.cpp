...
{ 
	...
	CWAVChunk *chunk = mainContainer->find('data');
	if (chunk == 0)
		return toLongClamped(CTools::FileSize);
	return toLongClamped(CTools::FileSize - (__int64)chunk->getSize() + 8);
};
...
bool CWAVFile::SaveToFile(LPCWSTR FileName)
{
	...
	if (!newData->load(Source, CTools::ID3v2Size, toU32Clamped(CTools::FileSize -  CTools::ID3v1Size)))
	{
