#!/usr/bin/env python3
"""Creates tests/misuse_calls.inc from the C++ wrapper header: for every exported function (except save/remove/analyze)
a block that calls it with random, partly invalid arguments (NULL, empty, huge, index 0/-1/32767, frame IDs
...
SKIP = re.compile(r"(Save|Remove|Analyze|ImportCue)")     # change files or are a test of their own
...
def main():
    lines = ["// GENERATED by tests/gen_misuse.py - do not edit by hand", ""]
    count = 0
    for m in FUNC.finditer(HEADER.read_text(encoding="latin-1")):
        ...
        while i < len(ps):
            ...
            v = f"a{i}"
            if typ == "BYTE*":
                # buffer + length: the length never exceeds the buffer (that would be a caller error)
        ...
        count += 1
    OUT.write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(f"{count} functions -> {OUT.name}")
