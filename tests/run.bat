...
cl /nologo /EHsc /utf-8 /wd4828 /std:c++17 /O1 /W3 /MD /I"%TESTS%third_party" /DAG3_DEF_PATH=\"%ROOT:\=/%/AudioGenie3.def\" /Fe:ag3tests.exe "%TESTS%support.cpp" "%TESTS%test_exports.cpp" "%TESTS%test_wav.cpp" "%TESTS%test_mpeg.cpp" "%TESTS%test_robustness.cpp" catch2.obj /link "%DLLDIR%\AudioGenie3.lib" oleaut32.lib || goto :fail
