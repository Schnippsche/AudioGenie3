// Robustness against invalid arguments: every exported function (except save/remove) is called with random,
// partly invalid values (NULL, empty/huge strings, index 0/-1/32767, unknown frame IDs), each on one analyzed
// file of every format. Expectation: no crash (under ASan: no memory error).
// The call list is in misuse_calls.inc (generated by gen_misuse.py); AG3_FUZZ_ROUNDS increases the calls per function.
...
// AG3_MISUSE_TRACE=1: write every call to %TEMP%/ag3tests/misuse_trace.log before executing it (search for crash causes)
...
const wchar_t* pickString(Rng& r, bool isFileName)
{
    ...
    // file names: only those that cannot create anything anywhere (some functions write files)
...
{
    // the buffer is always 4096 bytes; the length never exceeds it (that would be a caller error). NULL only with length 0.
...
// set a few frames so that the getters find something
void prepareId3v2()
{
...
void battery(const std::string& name, const fs::path& file, uint32_t seed)
{
    ...
    prepareId3v2();
    int called = 0;
    const std::string only;   // empty = all functions
    (void)only;
#define name only
#include "misuse_calls.inc"
#undef name
    REQUIRE(called > 300);   // the list covers (almost) the whole API
    (void)name;
}

}  // namespace

TEST_CASE("Invalid arguments: all functions on one file per format", "[misuse]")
{
